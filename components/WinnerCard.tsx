...
export function WinnerCard({ w }: { w: Winner }) {
  ...
  return (
    <div className="glass-panel rim-glow overflow-hidden group transition-transform duration-300 hover:-translate-y-1">
      <div className="relative">
        ...
        <span className="absolute top-2.5 right-2.5 font-mono-num text-[11px] font-extrabold px-2 py-1 rounded-md bg-black/65 text-white border border-white/10 backdrop-blur-md">
          #{w.winning_ticket_number}
        </span>
      </div>
      <div className="p-4 relative">
        <div className="font-display font-bold text-white text-base leading-snug line-clamp-2 tracking-tight">{w.prize_title}</div>
        <div className="text-sm text-white/75 mt-1.5">
          {w.display_name}{w.display_location ? <span className="text-white/55">, {w.display_location}</span> : null}
        </div>
        <div className="mt-3 flex items-center justify-between">
          <div className="text-[11px] text-white/50 font-mono-num">Drawn {new Date(w.draw_date).toLocaleDateString()}</div>
          {w.proof_url ? (
