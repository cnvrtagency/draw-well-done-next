...
const toneRing: Record<Tone, string> = {
  default: "from-[var(--td-surface-hover)] to-transparent",
    ...
    <div className={cn("relative glass-panel overflow-hidden p-2.5 md:p-5", className)}>
      ...
      <div className="relative flex h-full min-h-[56px] items-center justify-between gap-2 md:min-h-[86px] md:items-start md:gap-3">
        <div className="flex min-w-0 flex-col gap-1 md:self-stretch">
          <div className="truncate text-[9px] font-extrabold uppercase tracking-[0.12em] td-soft md:text-[11px] md:tracking-[0.16em]">{label}</div>
          <div>
            <div className="font-mono-num text-lg font-extrabold leading-none td-text md:mt-2 md:text-3xl">{value}</div>
            {hint ? <div className="mt-0.5 text-[10px] td-soft md:mt-1.5 md:text-[11px]">{hint}</div> : null}
          </div>
        </div>
        {icon ? <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg border td-border bg-[color:var(--td-surface-muted)] text-primary md:h-10 md:w-10 md:rounded-xl">{icon}</div> : null}
      </div>
    </div>
  );
}
