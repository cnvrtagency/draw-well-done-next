  ...
  return (
    <div>
      ...
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.to + l.label}>
            <Link href={l.to} className="font-extrabold uppercase text-[11px] tracking-wider text-white/65 hover:text-white transition-colors">{l.label}</Link>
          </li>
        ))}
      </ul>
    </div>
...
export function Footer() {
  return (
    <footer className="relative mt-20 border-t border-white/10 bg-gradient-to-b from-background to-[hsl(222_50%_2%)] overflow-hidden">
      ...
      <div className="border-b border-white/10 bg-white/[0.02] relative">
      ...
      <div className="container mx-auto px-4 py-12 grid gap-10 md:grid-cols-[1fr_2fr] text-sm relative">
        <div>
          <img src="/assets/topdraw-logo.png" alt={`${brand.name} logo`} width={470} height={95} className="h-9 w-auto mb-3" />
          <p className="text-white/85 text-sm leading-relaxed">UK prize competitions, drawn fairly.</p>
          <p className="text-white/55 text-xs leading-relaxed mt-3">Independent UK prize competitions with published winners, transparent entry caps and a genuine free postal entry route.</p>
          <a href={`mailto:${brand.supportEmail}`} className="inline-flex items-center gap-2 text-white/70 hover:text-white text-xs mt-4"><Mail className="w-3.5 h-3.5" /> {brand.supportEmail}</a>
        </div>
