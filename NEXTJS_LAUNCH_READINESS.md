...
- `/account/entries` now uses scoped `account-entry-card`, `account-entry-title`, `account-entry-meta`, `account-entry-date`, `account-ticket-pill` and `account-ticket-pill-active` classes so competition titles, ticket counts, draw dates, ticket pills and card borders remain readable in light mode without altering ticket grouping, status badges, routes or data logic.
- `/account/orders`, `/account/transactions` and `/account/wallet` now use scoped account metadata utilities for dates, entry counts, notes and subdued balances where generic `td-soft`/`td-muted` were too weak for functional data in light mode.
- `Header` now uses a more premium glass command-bar treatment in both themes: tokenised shell, nav tabs, active underline/pill, compact control chips and mobile drawer link styling improve the visual design while preserving the existing logo/nav/wallet/MiniCart/theme/account/admin/login behavior.
