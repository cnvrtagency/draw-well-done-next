...
- Account entries light mode now uses scoped `account-entry-*` classes in `AccountPages.tsx` plus light-only CSS overrides in `app/globals.css` so competition titles, ticket counts, draw dates, ticket pills and card surfaces stay readable without globally changing `td-soft` or `td-muted`.
- Related `/account/orders`, `/account/transactions` and `/account/wallet` metadata rows now use scoped account metadata utilities for dates, entry counts, notes and subdued balances where the generic soft tokens were too faint for functional account information.
- `Header` now uses a more premium tokenised shell/navigation/control treatment: `td-header-shell`, `td-header-nav`, `td-header-nav-link`, `td-header-control` and `td-header-mobile-menu` create a glass command-bar feel in dark mode and a silver/light-glass command-bar feel in light mode without changing routing, auth, wallet, MiniCart or theme logic.
